...
export class AuthenticationService {
    ...
    public async register(payload: { email: string, password: string }): Promise<string> {
        console.log(payload);

        const text = `INSERT INTO users(email, password) VALUES ($1, $2)`;
        try {
            const hashPassword = Crypto.createHash('sha256').update(payload.password).digest('base64');
